...
<!--
    This template is used to render the list content for the Target Tracking module.
    It includes a table for displaying target products and their competitors.
    The JavaScript handles showing and hiding competitors dynamically.
    -->
...
{block name="td_content"}
    {if $key == 'competitors'}
        <div class="competitors-inline-list">
            <table class="table competitors-table-{$tr.id_target_website}">
                <thead>
                <tr>
                    <th>Priority</th>
                    <th>Logo</th>
                ...
                <tbody>
                {assign var=competitors value=TargetCompetitorRelation::getCompetitorsByTarget($tr.id_target_website)}
                ...
                {if $competitors}
                    {foreach from=$competitors item=competitor name=compLoop}
                        ...
                        <tr class="competitor-row priority-{$competitor.priority}"
                            ...
                            <td class="fixed-width-md"><a href="{$competitor.url|escape:'html':'UTF-8'}"
                                                          target="_blank">{$competitor.name|capitalize|escape:'html':'UTF-8'}</a>
                            </td>

                            <td class="text-right">
                                {if $activeCount == 1 && $competitors|@count > 1}
                                    <button class="show-more-btn" data-product-id="{$id_target_website}">
