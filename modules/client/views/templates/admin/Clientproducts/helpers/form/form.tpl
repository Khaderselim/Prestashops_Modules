...
{*                                    Logo*}
                                    ...
                                    <div class="col-lg-10">
                                        <div class="form-group">
                                            <label>{l s='URL'}</label>
                                            <input type="url" class="form-control" name="competitors[{$competitor.id_product}][url]" value="{$competitor.url}">
                                            <input type="hidden" name="competitors[{$competitor.id_product}][id_product]" value="{$competitor.id_product}">
                                        </div>
                                    </div>
                                    <div class="col-lg-1">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group" >
                                                <button type="button" class="btn btn-danger delete-competitor" data-id="{$competitor.id_product}">
...
// Update the competitor row template in the JavaScript
            var newRow = $('<div class="competitor-row well clearfix">' +
                ...
                var nameInput = newRow.find('.name-input');

                urlInput.on('change', function() {
                    var url = $(this).val();
                    if (url) {
                        // AJAX call to check if competitor exists
                        $.ajax({
    ...
    {elseif $input.type == 'search'}
        <div class="form-group">
            ...
            <div class="input-groups">
                <input type="text" class="form-control col-lg-12" placeholder="{l s='Type to search...'}" value="{if isset($fields_value.search)}{$fields_value.search|escape:'html':'UTF-8'}{/if}" autocomplete="off" name="search">
