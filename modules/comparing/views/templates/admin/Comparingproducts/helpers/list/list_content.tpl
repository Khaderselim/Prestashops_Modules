...
{block name="td_content"}
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $(document).off('click', '.add-product').on('click', '.add-product', function (e) {
                    ...
                    $('#targetWebsiteModal').modal('show');
                });

                // Add confirm handler
                $('#confirmAdd').off('click').on('click', function () {
                    var idTargetWebsite = $('#target_website').val();
                    var button = $(this);

                    $.ajax({
                        url: '{$link->getAdminLink('AdminComparingproducts')}&ajax=1&action=addCompare',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            id_product: selectedProductId,
                            id_competitor_product: selectedCompetitorId,
                            id_target_website: idTargetWebsite
                        },
                        beforeSend: function() {
    ...
    {/if}
    <!-- Modal -->
    <div id="targetWebsiteModal" class="bootstrap modal fade" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h3 class="modal-title">{l s='Select Target Website'}</h3>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label class="control-label">{l s='Target Website'}</label>
                        <select name="id_target_website" id="target_website" class="form-control">
                            {foreach from=$target_websites item=website}
                                <option value="{$website.id_target_website}">{$website.name}</option>
                            {/foreach}
                        </select>
                    </div>
                </div>
