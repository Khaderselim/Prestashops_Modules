...
require_once _PS_MODULE_DIR_ . 'comparing/classes/SuggestionProducts.php';
...
class AdminComparingproductsController extends ModuleAdminController{
    ...
    {
        parent::initContent();
        $target_websites = Db::getInstance()->executeS('
        SELECT id_target_website, name 
        FROM `'._DB_PREFIX_.'target_websites` 
        WHERE active = 1
    ');
        ...
        $suggestion_helper->table = 'suggestion_product';
        ...
        $suggestionQuery->from('suggestion_product');
    ...
    public function addCompare()
    {
        ...
        try {
            ...
            $suggestion_value = Db::getInstance()->getRow('SELECT * FROM '._DB_PREFIX_.'suggestion_product WHERE id_product = '.(int)$id_product .' AND id_competitor_product = '.(int)$id_competitor_product);
            // Debug log
            $id_target_product = Db::getInstance()->getValue('SELECT id_target_product FROM '._DB_PREFIX_.'targets_products WHERE id_product = '.(int)$id_product .' AND id_target_website = '.(int)$id_target_website);
            ...
            ');
                ...
                if($product){
                    ...
                    Db::getInstance()->insert('targets_products', array(
                        'id_product' => (int)$id_product,
                        'id_target_website' => (int)$id_target_website,
            ...
            Db::getInstance()->insert('comparing_product', array(
            ...
            Db::getInstance()->insert('target_products_relation', array(
                'id_product_target' => (int)$id_target_product,
                'id_product_competitor' => (int)$id_competitor_product,
            ));
            // Send response
            $response['success'] = true;
            $response['message'] = 'Product added successfully' . ' with ID: ' . $id_product . ' and Competitor Product ID: ' . $id_competitor_product . ' and Target Website ID: ' . $id_target_website;
